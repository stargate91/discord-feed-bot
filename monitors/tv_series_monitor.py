...
class TVSeriesMonitor(BaseMonitor):
    ...
    async def check_for_updates(self):
        ...
        for series in reversed(feed):
            ...
            if not await database.is_published(series_id, "tv_series"):
                if self.is_first_run:
                    await database.mark_as_published(series_id, "tv_series", self.api_url)
                else:
                    new_entries.append(series)
                    log.info(f"New TV Series update: {series.get('name')} ({series_id})")
        ...
        for series in new_entries:
            ...
            embed.set_footer(text=f"TMDB • First Air Date: {first_air_date}")
    ...
    async def get_latest_item(self):
        """Fetch the most recent TV series update."""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.get_api_url(), headers=self.get_headers()) as response:
                    ...
                    embed = discord.Embed(
                        title=name[:256],
                        url=tmdb_url,
                        description=series.get("overview", "")[:300],
                        color=self.get_color(0x01d277)
                    )
                    poster_path = series.get("poster_path")
                    if poster_path:
